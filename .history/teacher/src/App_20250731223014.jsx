...
const counts=Array.from({length:100})
  ...
  return (
    <div className='bg-black transition-colors animate-bounce '>
    {
      counts.map((i,index)=>{
        return(
          index%5==0?<div key={i} className='w-full  h-2 transition-transform duration-500  bg-amber-500'></div>:<div key={i} className='w-full h-2 transition-transform duration-500    bg-green-700'></div>
