...
const Dashboard = () => {
  ...
  const[url,setUrl]=useState("http://localhost:4000/api/dashboard/summary")
   useEffect(()=>{
        setActiveComponent('overview');
        
   },[url])
  useEffect(() => {
    // Fetch all counts from your backend API
  ...
  const renderComponent = () => {
    switch (activeComponent) {
      ...
      case 'viewTeachers':
        return <div>ViewAllTeachers Component</div>;
      case 'viewStudents':
...
const DashboardOverview = ({ stats, onActionClick }) => {
  ...
  return (
    <div>
      ...
      {/* Optional: Show actions separately */}
      <div className='mt-8'>
        <h3 className='text-xl font-semibold mb-3'>Quick Actions</h3>
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
          <ActionButton label="Add Teacher" onClick={() => onActionClick('addTeacher')} />
          <ActionButton label="Add Student" onClick={() => onActionClick('addStudent')} />
          <ActionButton label="Add Class" onClick={() => onActionClick('addClass')} />
        </div>
      </div>
    </div>
  );
